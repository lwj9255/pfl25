...
class Server_fed03(Server):
    ...
    def get_personalized_parameters(self):
        ...
        for cid, template in zip(self.uploaded_ids, self.uploaded_models):
            # 取出对应的 latent_code，其长度为 (T+1)*param_dim，因此可以计算出每个“子向量”的维度 param_dim
            latent_code = self.latent_encodings[cid]  # shape: ( (T+1)*param_dim, )，T个噪音，1个最终加噪参数
            ...
            # 随后依次按块提取存储的噪声
            # 注意提取顺序为：第一个块为 eps_T，第二个为 eps_{T-1}……
            # 存储顺序与前向扩散时逆序拼接一致
            noise_list = []
            for step_idx in range(self.diff_steps):
                # 每次取param_dim个，也就是一步噪音的大小
            ...
            # 现在多步反向
            cur_x = x_T.view(-1)  # 初始令 cur_x 等于 x_T
            for k in range(self.diff_steps):
                # 依次对 k 从 0 到 T-1 执行反向扩散：
                # 对应第 k 步中，取出噪声 eps_k（注意噪声的顺序已经是 eps_T, eps_{T-1}, …）。
                eps_k = noise_list[k]  # = eps_{T-k}, k=0->eps_T
                # 这里 beta 的下标通过 beta_idx = T - 1 - k 得到。
